...
void main() {\r
  ...
  vec4 rgba = vColor * abs(sin(time));\r
  // フラグメントの色\r
  gl_FragColor = vec4(rgba);\r
}\r
\r
`,p=Math.PI*-.5,a=.2,h=.4,d=[],b=[];for(let i=0;i<5;i++){const t=p+.4*Math.PI*i,e=p+360/5/180*Math.PI*(i+.5),n=p+360/5/180*Math.PI*(i+1),r=[0,0,0],s=[Math.cos(t)*a,Math.sin(t)*a,0],l=[Math.cos(e)*h,Math.sin(e)*h,0],v=[0,0,0],f=[Math.cos(n)*a,Math.sin(n)*a,0],S=[Math.cos(e)*h,Math.sin(e)*h,0];d.push(...r,...s,...l,...v,...f,...S);const A=[255/255,245/255,23/255,1,255/255,177/255,0/255,1,255/255,248/255,83/255,1,255/255,245/255,23/255,1,255/255,177/255,0/255,1,255/255,248/255,83/255,1];b.push(...A)}const B=`attribute vec3 position;\r
attribute vec4 color;\r
varying vec4 vColor;\r
\r
void main() {\r
...
void main() {\r
  ...
  vec4 rgba = vColor * abs(cos(time));\r
  // フラグメントの色\r
  gl_FragColor = vec4(rgba);\r
}\r
\r
`,g=Math.PI*.5,c=.2,u=.25,m=[],_=[];for(let i=0;i<5;i++){const t=g+.4*Math.PI*i,e=g+360/5/180*Math.PI*(i+1),n=[0+u,0,0],r=[Math.cos(t)*c+u,Math.sin(t)*c,0],s=[Math.cos(e)*c+u,Math.sin(e)*c,0];m.push(...n,...r,...s);const l=[255/255,245/255,23/255,1,255/255,177/255,0/255,1,255/255,248/255,83/255,1];_.push(...l)}class T{constructor(){this.canvas=null,this.gl=null,this.program=null,this.program_penta=null,this.uniformLocation=null,this.uniformLocation_penta=null,this.position=null,this.position_penta=null,this.positionStride=null,this.positionStride_penta=null,this.positionVBO=null,this.positionVBO_penta=null,this.color=null,this.color_penta=null,this.colorStride=null,this.colorStride_penta=null,this.colorVBO=null,this.colorVBO_penta=null,this.startTime=0,this.currentTime=0,this.elapsedTime=0,this.delta=0,this.isRender=!1,this.render=this.render.bind(this),this.guiParam={},this.attPosition=null,this.attColor=null,this.attPosition_penta=null,this.attColor_penta=null}init(){this.startTime=Date.now(),this.canvas=document.getElementById("webgl-canvas"),this.gl=o.createWebGLContext(this.canvas);const t=Math.min(window.innerWidth,window.innerHeight);this.canvas.width=t,this.canvas.height=t}load(){return new Promise((t,e)=>{t()})}createProgram(){const t=this.gl;if(t==null){const e=new Error("not initialized");reject(e)}else{const e=o.createShaderObject(t,P,t.VERTEX_SHADER),n=o.createShaderObject(t,B,t.VERTEX_SHADER),r=o.createShaderObject(t,O,t.FRAGMENT_SHADER),s=o.createShaderObject(t,C,t.FRAGMENT_SHADER);this.program=o.createProgramObject(t,e,r),this.program_penta=o.createProgramObject(t,n,s)}}setupMesh(){const t=this.gl;this.positionStride=3,this.vertexCount=d.length/this.positionStride,this.positionVBO=o.createVBO(this.gl,d),this.colorStride=4,this.colorVBO=o.createVBO(this.gl,b),this.positionStride_penta=3,this.vertexCount_penta=m.length/this.positionStride_penta,this.positionVBO_penta=o.createVBO(this.gl,m),this.colorStride_penta=4,this.colorVBO_penta=o.createVBO(this.gl,_),this.attPosition=t.getAttribLocation(this.program,"position"),this.attColor=t.getAttribLocation(this.program,"color"),o.enableAttribute(t,this.positionVBO,this.attPosition,this.positionStride),o.enableAttribute(t,this.colorVBO,this.attColor,this.colorStride),this.uniformLocation={time:t.getUniformLocation(this.program,"time")},this.attPosition_penta=t.getAttribLocation(this.program_penta,"position"),this.attColor_penta=t.getAttribLocation(this.program_penta,"color"),o.enableAttribute(t,this.positionVBO_penta,this.attPosition_penta,this.positionStride_penta),o.enableAttribute(t,this.colorVBO_penta,this.attColor_penta,this.colorStride_penta),this.uniformLocation_penta={time:t.getUniformLocation(this.program_penta,"time")}}setBlending(t){const e=this.gl;e!=null&&(t===!0?e.enable(e.BLEND):e.disable(e.BLEND))}setupRendering(){const t=this.gl;t.viewport(0,0,this.canvas.width,this.canvas.height),t.clearColor(.3,.3,.3,1),t.clear(t.COLOR_BUFFER_BIT),t.blendFuncSeparate(t.SRC_ALPHA,t.ONE_MINUS_SRC_ALPHA,t.ONE,t.ONE)}setupGUI(){const t=new R,e={};e.toggleRender=()=>{this.isRender=!this.isRender,this.isRender&&this.start()},t.add(e,"toggleRender")}start(){this.isRender=!0,this.render()}stop(){this.isRender=!1}render(){const t=this.gl;this.isRender===!0&&requestAnimationFrame(this.render),this.setupRendering(),this.currentTime=Date.now(),this.delta=Math.max(0,Math.min(.032,(this.currentTime-this.startTime)/1e3)),this.elapsedTime+=this.delta,this.startTime=this.currentTime,t.useProgram(this.program),t.uniform1f(this.uniformLocation.time,this.elapsedTime),o.enableAttribute(t,this.positionVBO,this.attPosition,this.positionStride),o.enableAttribute(t,this.colorVBO,this.attColor,this.colorStride),t.drawArrays(t.TRIANGLES,0,this.vertexCount),t.useProgram(this.program_penta),t.uniform1f(this.uniformLocation_penta.time,this.elapsedTime),o.enableAttribute(t,this.positionVBO_penta,this.attPosition_penta,this.positionStride_penta),o.enableAttribute(t,this.colorVBO_penta,this.attColor_penta,this.colorStride_penta),t.drawArrays(t.TRIANGLES,0,this.vertexCount_penta)}}window.addEventListener("DOMContentLoaded",()=>{const i=new T;i.init(),i.load().then(()=>{i.createProgram(),i.setupMesh(),i.setBlending(!0),i.setupGUI(),i.start()})},!1);
